    ...
    <div class="bg-white rounded shadow-md border border-gray-300">
        ...
        <div class="p-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
                <!-- Search & Filter Form -->
                <form method="GET" action="{{ route('attendance.archive') }}" class="flex flex-wrap gap-2 items-center justify-between w-full">
                    ...
                    <div class="flex flex-wrap gap-2 items-center">
                        ...
                        @if(request('search') || request('event_id') || request('date_filter'))
                            <a href="{{ route('attendance.archive') }}?per_page={{ request('per_page', 10) }}" class="text-xs text-gray-500 underline ml-2">Reset</a>
                        @endif
                    </div>
                </form>
            </div>
                ...
                <table class="min-w-full border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="text-xs hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4">{{ $attendance->date ? date('d M Y', strtotime($attendance->date)) : '-' }}</td>
                            <td class="py-3 px-4">{{ $attendance->start_time ? date('H:i', strtotime($attendance->start_time)) : '-' }} - {{ $attendance->end_time ? date('H:i', strtotime($attendance->end_time)) : '-' }}</td>
                            <td class="py-3 px-4">
                                <span class="bg-status-completed-bg text-status-completed-text px-2 py-1 rounded-full text-xs">

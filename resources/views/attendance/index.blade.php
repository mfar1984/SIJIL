<x-app-layout>
    ...
    <div class="bg-white rounded shadow-md border border-gray-300">
            ...
            <div class="flex justify-between items-start">
                ...
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route('attendance.archive') }}" class="bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 hover:to-orange-600 text-white px-3 h-[36px] rounded shadow-sm font-medium flex items-center text-xs transition-colors duration-200 ease-in-out">
                        <span class="material-icons-outlined text-xs mr-1">inventory</span>
                        View Archive
                    </a>
                    @endcan
        ...
        <div class="p-4">
            ...
            <form method="GET" action="{{ route('attendance.index') }}" class="flex flex-wrap items-center gap-2 mb-4">
                ...
                @if(request('search') || request('status') || request('event_id') || request('date_filter'))
                    <a href="{{ route('attendance.index') }}" class="text-xs text-gray-500 underline shrink-0">Reset</a>
                @endif
            </form>
                            ...
                            <td class="py-3 px-4">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('attendance.qrcode', $attendance->id) }}" class="p-1 bg-green-50 rounded hover:bg-green-100 border border-green-100" title="View QR Code" target="_blank">
                                        <span class="material-icons-outlined text-green-600 text-xs">qr_code</span>
                                    </a>
                                    ...
                                    @if($attendance->status !== 'archived')
                                        ...
                                        <form method="POST" action="{{ route('attendance.archive-action', $attendance->id) }}" style="display:inline;">
                                            ...
                                            <button type="submit" class="p-1 bg-warning-50 rounded hover:bg-warning-100 border border-warning-100" onclick="return confirm('Archive this attendance?')" title="Archive">
                                                <span class="material-icons-outlined text-warning-600 text-xs">archive</span>
                                            </button>
                                        </form>
                                        @endcan
                                    @else

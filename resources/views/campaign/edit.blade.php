<x-app-layout>
    ...
    <div class="bg-white rounded shadow-md border border-gray-300">
        ...
        <div class="p-6">
            @php $canUpdate = auth()->user()->can('campaigns.update'); @endphp
            <form action="{{ route('campaign.update', ['campaign' => $campaign->id]) }}" method="POST" class="space-y-6" id="campaignForm">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <textarea id="campaign_description" name="campaign_description" rows="3" class="w-full text-xs border-gray-300 rounded-[1px] py-3 focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50">{{ $campaign->description }}</textarea>
                            </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none {{ $campaign->campaign_type == 'email' ? 'border-blue-500' : '' }}" id="email-campaign-label">
                            <input type="radio" name="campaign_type" value="email" class="sr-only" {{ $campaign->campaign_type == 'email' ? 'checked' : '' }}>
                        ...
                        <label class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none {{ $campaign->campaign_type == 'sms' ? 'border-blue-500' : 'hover:border-blue-500' }}" id="sms-campaign-label">
                            <input type="radio" name="campaign_type" value="sms" class="sr-only" {{ $campaign->campaign_type == 'sms' ? 'checked' : '' }}>
                            <span class="flex flex-1">
                                <span class="flex flex-col">
                                    <span class="flex items-center text-sm font-medium text-gray-900">
                ...
                <div class="mb-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <select id="audience_type" name="audience_type" class="w-full text-xs border-gray-300 rounded-[1px] pl-12 focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" required>
                                    ...
                                    <option value="specific_event" {{ $campaign->audience_type == 'specific_event' ? 'selected' : '' }}>Specific Event</option>
                                    <option value="custom_filter" {{ $campaign->audience_type == 'custom_filter' ? 'selected' : '' }}>Custom Filter</option>
                                    <option value="custom_emails" {{ $campaign->audience_type == 'custom_emails' ? 'selected' : '' }}>Custom</option>
                                </select>
                            </div>
                            ...
                            <div class="relative">
                                ...
                                <select id="event_id" name="event_id" class="w-full text-xs border-gray-300 rounded-[1px] pl-12 focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50">
                                    ...
                                    @foreach($events as $event)
                                        <option value="{{ $event->id }}" {{ ($campaign->event_id == $event->id) ? 'selected' : '' }}>{{ $event->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div id="custom_filter_options" class="{{ $campaign->audience_type == 'custom_filter' ? '' : 'hidden' }} space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select id="filter_age" name="filter_age" class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-DEFAULT focus:ring focus:ring-primary-DEFAULT focus:ring-opacity-50 text-sm">
                                        ...
                                        <option value="18-24" {{ isset($campaign->filter_criteria['age']) && $campaign->filter_criteria['age'] == '18-24' ? 'selected' : '' }}>18-24</option>
                                        <option value="25-34" {{ isset($campaign->filter_criteria['age']) && $campaign->filter_criteria['age'] == '25-34' ? 'selected' : '' }}>25-34</option>
                                        <option value="35-44" {{ isset($campaign->filter_criteria['age']) && $campaign->filter_criteria['age'] == '35-44' ? 'selected' : '' }}>35-44</option>
                                        <option value="45+" {{ isset($campaign->filter_criteria['age']) && $campaign->filter_criteria['age'] == '45+' ? 'selected' : '' }}>45+</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select id="filter_gender" name="filter_gender" class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-DEFAULT focus:ring focus:ring-primary-DEFAULT focus:ring-opacity-50 text-sm">
                                        ...
                                        <option value="male" {{ isset($campaign->filter_criteria['gender']) && $campaign->filter_criteria['gender'] == 'male' ? 'selected' : '' }}>Male</option>
                                        <option value="female" {{ isset($campaign->filter_criteria['gender']) && $campaign->filter_criteria['gender'] == 'female' ? 'selected' : '' }}>Female</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="filter_attendance" name="filter_attendance" class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-DEFAULT focus:ring focus:ring-primary-DEFAULT focus:ring-opacity-50 text-sm">
                                    ...
                                    <option value="attended" {{ isset($campaign->filter_criteria['attendance']) && $campaign->filter_criteria['attendance'] == 'attended' ? 'selected' : '' }}>Attended</option>
                                    <option value="not_attended" {{ isset($campaign->filter_criteria['attendance']) && $campaign->filter_criteria['attendance'] == 'not_attended' ? 'selected' : '' }}>Not Attended</option>
                                    <option value="registered" {{ isset($campaign->filter_criteria['attendance']) && $campaign->filter_criteria['attendance'] == 'registered' ? 'selected' : '' }}>Registered Only</option>
                                </select>
                            </div>
                        </div>
                        
                        <div id="custom_emails_input" class="{{ $campaign->audience_type == 'custom_emails' ? '' : 'hidden' }}">
                            ...
                            <textarea id="custom_emails" name="custom_emails" rows="3" class="w-full text-xs border-gray-300 rounded focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" placeholder="Enter email addresses separated by commas, e.g. test@example.com, user@domain.com">{{ $campaign->custom_emails ?? '' }}</textarea>
                ...
                <div class="mb-6" id="email_subject_section">
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" id="email_subject" name="email_subject" class="w-full text-xs border-gray-300 rounded-[1px] focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" value="{{ isset($campaign->content['subject']) ? $campaign->content['subject'] : old('email_subject', '') }}" required>
                            </div>
                ...
                <div id="email_content" class="mb-6 {{ $campaign->campaign_type == 'email' ? '' : 'hidden' }}">
                    ...
                    <div class="space-y-4">                        
                        ...
                        <div>
                            ...
                            <div class="border border-gray-300 rounded-md">
                                <textarea id="email_content" name="email_content" rows="15" class="w-full border-0 bg-gray-50 focus:ring-0 text-sm hidden">{{ isset($campaign->content['body']) ? $campaign->content['body'] : '' }}</textarea>
                            </div>
                            <p class="mt-1 text-[10px] text-gray-500">Compose the body of your email with formatting options.</p>
                        </div>
                        
                        <div class="flex items-center">
                            <input type="checkbox" id="include_unsubscribe" name="include_unsubscribe" class="rounded border-gray-300 text-primary-DEFAULT shadow-sm focus:border-primary-DEFAULT focus:ring focus:ring-primary-DEFAULT focus:ring-opacity-50" {{ isset($campaign->content['include_unsubscribe']) && $campaign->content['include_unsubscribe'] ? 'checked' : '' }}>
                            <label for="include_unsubscribe" class="ml-2 block text-xs text-gray-700">Include unsubscribe link (recommended)</label>
                        </div>
                    </div>
                </div>
                
                <!-- SMS Content (hidden by default) -->
                <div id="sms_content" class="mb-6 {{ $campaign->campaign_type == 'sms' ? '' : 'hidden' }}">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea id="sms_message" name="sms_message" rows="3" class="w-full text-xs border-gray-300 rounded focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" placeholder="Enter SMS message (160 characters max)">{{ isset($campaign->content['message']) ? $campaign->content['message'] : '' }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" id="include_shortlink" name="include_shortlink" class="rounded border-gray-300 text-primary-DEFAULT shadow-sm focus:border-primary-DEFAULT focus:ring focus:ring-primary-DEFAULT focus:ring-opacity-50" {{ isset($campaign->content['include_shortlink']) && $campaign->content['include_shortlink'] ? 'checked' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center">
                                    <input type="radio" id="send_now" name="schedule_type" value="now" class="focus:ring-primary-DEFAULT h-4 w-4 text-primary-DEFAULT border-gray-300" {{ $campaign->schedule_type == 'now' ? 'checked' : '' }}>
                                    <label for="send_now" class="ml-2 block text-sm text-gray-700">Send immediately</label>
                                </div>
                                <div class="flex items-center">
                                    <input type="radio" id="send_scheduled" name="schedule_type" value="scheduled" class="focus:ring-primary-DEFAULT h-4 w-4 text-primary-DEFAULT border-gray-300" {{ $campaign->schedule_type == 'scheduled' ? 'checked' : '' }}>
                                    <label for="send_scheduled" class="ml-2 block text-sm text-gray-700">Schedule for later</label>
                                </div>
                            </div>
                        </div>
                        
                        <div id="scheduled_options" class="{{ $campaign->schedule_type == 'scheduled' ? '' : 'hidden' }}">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="scheduled_date" class="block text-xs font-medium text-gray-700 mb-1">Date</label>
                                    <input type="date" id="scheduled_date" name="scheduled_date" class="w-full h-9 rounded-md border-gray-300 shadow-sm focus:border-primary-DEFAULT focus:ring focus:ring-primary-DEFAULT focus:ring-opacity-50 text-sm" value="{{ $campaign->scheduled_at ? $campaign->scheduled_at->format('Y-m-d') : '' }}">
                                </div>
                                
                                <div>
                                    <label for="scheduled_time" class="block text-xs font-medium text-gray-700 mb-1">Time</label>
                                    <input type="time" id="scheduled_time" name="scheduled_time" class="w-full h-9 rounded-md border-gray-300 shadow-sm focus:border-primary-DEFAULT focus:ring focus:ring-primary-DEFAULT focus:ring-opacity-50 text-sm" value="{{ $campaign->scheduled_at ? $campaign->scheduled_at->format('H:i') : '' }}">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        // Store the email content from PHP to JavaScript variable
        const emailBodyContent = @json(isset($campaign->content['body']) ? $campaign->content['body'] : '');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            tinymce.init({
                ...
                init_instance_callback: function(editor) {
                    // Set the content directly from our JavaScript variable
                    editor.setContent(emailBodyContent);
                    // Email content from database
                }
            });

    ...
    <div class="bg-white rounded shadow-md border border-gray-300">
        ...
        <div class="p-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
                <!-- Search & Filter Form -->
                <form method="GET" action="{{ route('pwa.participants') }}" class="flex flex-wrap gap-2 items-center justify-between w-full">
                    ...
                    <div class="flex flex-wrap gap-2 items-center">
                        ...
                        @if(request('search') || request('status'))
                            <a href="{{ route('pwa.participants') }}?per_page={{ request('per_page', 10) }}" class="text-xs text-gray-500 underline ml-2">Reset</a>
                        @endif
                    </div>
                </form>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($participants as $participant)
                            <tr class="text-xs hover:bg-gray-50">
                                ...
                                <td class="py-3 px-4">{{ $participant->phone ?? 'N/A' }}</td>
                                <td class="py-3 px-4">{{ $participant->organization ?? 'N/A' }}</td>
                                <td class="py-3 px-4">
                                    ...
                                    <div class="flex justify-center space-x-2">
                                        ...
                                        <form method="POST" action="{{ route('pwa.participants.reset-password', $participant) }}" onsubmit="return confirm('Are you sure you want to reset the password for {{ $participant->name }}?')" class="inline-block">
                                            ...
                                            <button type="submit" class="p-1 bg-purple-50 rounded hover:bg-purple-100 border border-purple-100" title="Reset Password">
                                                <span class="material-icons-outlined text-purple-600 text-xs">smartphone</span>
                                            </button>
                                        </form>
                                        @endcan

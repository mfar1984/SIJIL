    ...
    <div class="bg-white rounded shadow-md border border-gray-300">
        ...
        <div class="p-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-end mb-4 gap-2">
                <form method="GET" action="" class="flex flex-wrap gap-2 items-center justify-end w-full sm:w-auto">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name, email, phone, IC..." class="border border-gray-300 rounded px-2 py-1 text-xs focus:ring focus:ring-primary-light focus:border-primary-light" />
                    <select name="event" class="appearance-none px-3 py-1.5 pr-8 text-xs border border-gray-300 rounded focus:ring focus:ring-primary-light focus:border-primary-light bg-white bg-no-repeat bg-right max-w-[200px] truncate" style="background-image: url('data:image/svg+xml;charset=US-ASCII,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2216%22 height=%2216%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%23888%22 stroke-width=%222%22 stroke-linecap=%22round%22 stroke-linejoin=%22round%22><polyline points=%226 9 12 15 18 9%22></polyline></svg>'); background-position: right 0.75rem center; background-size: 1em;">
                        <option value="">All Events</option>
                        @foreach(App\Models\Event::all() as $event)
                            <option value="{{ $event->id }}" @if(request('event') == $event->id) selected @endif class="truncate">{{ $event->name }}</option>
                        @endforeach
                    </select>
                    <select name="status" class="appearance-none px-3 py-1.5 pr-8 text-xs border border-gray-300 rounded focus:ring focus:ring-primary-light focus:border-primary-light bg-white bg-no-repeat bg-right w-[120px]" style="background-image: url('data:image/svg+xml;charset=US-ASCII,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2216%22 height=%2216%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%23888%22 stroke-width=%222%22 stroke-linecap=%22round%22 stroke-linejoin=%22round%22><polyline points=%226 9 12 15 18 9%22></polyline></svg>'); background-position: right 0.75rem center; background-size: 1em;">
                    ...
                    @if(request('search') || request('event') || request('status'))
                        <a href="{{ route('participants') }}" class="text-xs text-gray-500 underline ml-2">Reset</a>
                    @endif
                </form>
            </div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex justify-end">
                    {{ $participants->links('components.pagination-modern') }}
                </div>
            </div>
        </div>
    </div>

                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="phone" id="phone" class="w-full text-xs border-gray-300 rounded-[1px] pl-12 focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" value="{{ old('phone', $participant->phone) }}" placeholder="+60123456789">
                            </div>
                        ...
                        @php
                            ...
                            $icValue = '';
                            $passportValue = '';
                            $orgValue = old('organization', $participant->organization);
                            if (strpos($orgValue, 'IC:') === 0) {
                                $idType = 'ic';
                                $icValue = trim(substr($orgValue, 3));
                            } elseif (strpos($orgValue, 'Passport:') === 0) {
                                $idType = 'passport';
                                $passportValue = trim(substr($orgValue, 9));
                            }
                        @endphp
                        <div>
                            ...
                            <div class="mb-2">
                                <select name="id_type" id="id_type" class="w-full text-xs border-gray-300 rounded-[1px] focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" onchange="toggleIdFields()">
                                    <option value="" {{ empty($idType) ? 'selected' : '' }}>-- Select IC / Passport --</option>
                                    <option value="ic" {{ $idType == 'ic' ? 'selected' : '' }}>Identity Card</option>
                                    <option value="passport" {{ $idType == 'passport' ? 'selected' : '' }}>Passport</option>
                                </select>
                            </div>
                            <div id="ic_field" class="relative {{ $idType != 'ic' ? 'hidden' : '' }}">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <span class="material-icons text-[#004aad] text-base">assignment_ind</span>
                                </div>
                                <input type="text" name="organization_ic" id="organization_ic" placeholder="000000-00-0000" class="w-full text-xs border-gray-300 rounded-[1px] pl-12 focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" value="{{ $icValue }}">
                            </div>
                            <div id="passport_field" class="relative {{ $idType != 'passport' ? 'hidden' : '' }}">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <span class="material-icons text-[#004aad] text-base">assignment_ind</span>
                                </div>
                                <input type="text" name="organization_passport" id="organization_passport" placeholder="Enter passport number" class="w-full text-xs border-gray-300 rounded-[1px] pl-12 focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50" value="{{ $passportValue }}">
                            </div>
                            <input type="hidden" name="organization" id="organization" value="{{ old('organization', $participant->organization) }}">

<x-app-layout>
    ...
    <div class="bg-white rounded shadow-md border border-gray-300">
        ...
        <div class="p-4">
            ...
            <form method="GET" action="{{ route('template.designer') }}" class="flex flex-wrap items-center gap-2 mb-4">
                ...
                @if(request('search') || request('orientation') || request('date_filter'))
                    <a href="{{ route('template.designer') }}" class="text-xs text-gray-500 underline shrink-0">Reset</a>
                @endif
            </form>
                            ...
                            <tr class="text-xs hover:bg-gray-50">
                                ...
                                <td class="py-3 px-4">
                                    <div class="flex justify-center space-x-2">
                                        ...
                                        @endcan
                                        {{-- Design opens the layout editor, which saves over the
                                             template, so it is an update and not a create. Gating it
                                             as a create showed the button to roles the route then
                                             rejected with a 403. --}}
                                        @can('templates.update')
                                        <a href="{{ route('template.designer.create', ['id' => $template->id]) }}" class="p-1 bg-purple-50 rounded hover:bg-purple-100 border border-purple-100" title="Design">
                                            <span class="material-icons-outlined text-purple-600 text-xs">design_services</span>
                                        </a>
                                        @endcan
                                        @can('templates.create')
                                        <form action="{{ route('template.duplicate', $template->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="p-1 bg-indigo-50 rounded hover:bg-indigo-100 border border-indigo-100" title="Duplicate">
                                                <span class="material-icons-outlined text-indigo-600 text-xs">content_copy</span>
                                            </button>
                                        </form>
                                        @endcan
                        ...
                        @empty
                            <tr class="text-xs">
                                <td colspan="5" class="py-8 text-center text-gray-500">
                                    No templates found.
                                    @can('templates.create')
                                        <a href="{{ route('template.create') }}" class="text-primary-DEFAULT hover:underline">Create your first template</a>
                                    @endcan
                                </td>
                            </tr>
                        @endforelse
